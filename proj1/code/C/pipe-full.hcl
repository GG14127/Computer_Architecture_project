...
#  We only added instruction iaddl here. The modifications are the same as what we have done in Part B.
...
#  Add IIADDL in the choice region of need_regids since iaddl operation involves one register. 
#  Add IIADDL in the choice region of need_valC since iaddl operation need constant parameter. 
#  Set the srcB as rB. 
#  Set the dstE as rB. Because iaddl adds the immediate number into the destination register and leave writes value into register %esp.
#  Set aluA as valC. It is the first operand of ALU.
#  Set aluB as valB. It is the second operand of ALU.
#  Add IIADDL in the choice region of set_cc since iaddl operation involves ALU operation which will set flags.
